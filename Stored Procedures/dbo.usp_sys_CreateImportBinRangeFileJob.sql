...
/**/
/**/
CREATE PROCEDURE [dbo].[usp_sys_CreateImportBinRangeFileJob]
@Add nvarchar(10) = N'Y',
...
BEGIN
  ...
  SET @Script='
    EXEC '+@DBName+'.dbo.usp_bin_LoadBINRangeFiles'

  DECLARE @JobName nvarchar(128)
  SET @JobName=N'Import BinRange File into '+@DBName
  ...
  IF @Add='Y' AND NOT EXISTS (SELECT job_id FROM msdb.dbo.sysjobs_view WHERE name = @JobName)
    EXEC [dbo].[usp_sys_CreateCommonJob] @Name=@JobName,@Script=@Script,@StratTime=50000,@FreqType=8,@FreqInterval=8,@FreqRecurrenceFactor=1
END
GO
