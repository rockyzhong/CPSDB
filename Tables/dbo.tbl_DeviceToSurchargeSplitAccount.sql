CREATE TABLE [dbo].[tbl_DeviceToSurchargeSplitAccount]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_DeviceToSurchargeSplitAccount_AuditLog] ON [dbo].[tbl_DeviceToSurchargeSplitAccount]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_DeviceToSurchargeSplitAccount'
...
GO
ALTER TABLE [dbo].[tbl_DeviceToSurchargeSplitAccount] ADD CONSTRAINT [pk_DeviceToSurchargeSplitAccount] PRIMARY KEY CLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_DeviceToSurchargeSplitAccount_DeviceId_EndDate_StartDate] ON [dbo].[tbl_DeviceToSurchargeSplitAccount] ([DeviceId], [EndDate], [StartDate]) ON [PRIMARY]
GO
