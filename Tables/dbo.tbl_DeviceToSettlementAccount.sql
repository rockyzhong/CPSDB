CREATE TABLE [dbo].[tbl_DeviceToSettlementAccount]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_DeviceToSettlementAccount_AuditLog] ON [dbo].[tbl_DeviceToSettlementAccount]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_DeviceToSettlementAccount'
...
GO
ALTER TABLE [dbo].[tbl_DeviceToSettlementAccount] ADD CONSTRAINT [pk_DeviceToSettlementAccount] PRIMARY KEY NONCLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [idx_DeviceToSettlementAccount_DeviceId_EndDate_StartDate] ON [dbo].[tbl_DeviceToSettlementAccount] ([DeviceId], [EndDate], [StartDate]) ON [PRIMARY]
GO
