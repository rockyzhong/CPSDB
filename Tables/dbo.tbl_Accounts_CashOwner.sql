CREATE TABLE [dbo].[tbl_Accounts_CashOwner]
(
...
CREATE TRIGGER [dbo].[tbl_Accounts_CashOwner_Auditlog] ON [dbo].[tbl_Accounts_CashOwner] WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_Accounts_CashOwner'
...
GO
ALTER TABLE [dbo].[tbl_Accounts_CashOwner] ADD CONSTRAINT [PK_tbl_Accounts_CashOwner] PRIMARY KEY NONCLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_Accounts_CashOwner_AccountID] ON [dbo].[tbl_Accounts_CashOwner] ([AccountId]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
