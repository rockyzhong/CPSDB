CREATE TABLE [dbo].[tbl_upm_UserToRole]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_upm_UserToRole_AuditLog] ON [dbo].[tbl_upm_UserToRole]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_upm_UserToRole'
...
GO
ALTER TABLE [dbo].[tbl_upm_UserToRole] ADD CONSTRAINT [pk_UserToRole] PRIMARY KEY CLUSTERED  ([Id]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_UserToRole_RoleId] ON [dbo].[tbl_upm_UserToRole] ([RoleId]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_UserToRole_UserId] ON [dbo].[tbl_upm_UserToRole] ([UserId]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
