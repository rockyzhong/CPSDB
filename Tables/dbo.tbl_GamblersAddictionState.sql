CREATE TABLE [dbo].[tbl_GamblersAddictionState]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_GamblersAddictionState_AuditLog] ON [dbo].[tbl_GamblersAddictionState]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_GamblersAddictionState'
...
GO
ALTER TABLE [dbo].[tbl_GamblersAddictionState] ADD CONSTRAINT [pk_GamblersAddictionState] PRIMARY KEY CLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_GamblersAddictionState_RegionId] ON [dbo].[tbl_GamblersAddictionState] ([RegionId]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [idx_GamblersAddictionState_RegionId_IdState_IdNumber] ON [dbo].[tbl_GamblersAddictionState] ([RegionId], [IdState], [IdNumber]) ON [PRIMARY]
GO
