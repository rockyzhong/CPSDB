CREATE TABLE [dbo].[tbl_upm_UserSetting]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_upm_UserSetting_AuditLog] ON [dbo].[tbl_upm_UserSetting]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_upm_UserSetting'
...
GO
ALTER TABLE [dbo].[tbl_upm_UserSetting] ADD CONSTRAINT [pk_UserSetting] PRIMARY KEY CLUSTERED  ([Id]) ON [PRIMARY]
GO
