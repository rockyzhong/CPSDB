CREATE TABLE [dbo].[tbl_InterchangeScheme]
(
[Id] [bigint] NOT NULL IDENTITY(1, 2) NOT FOR REPLICATION,
[InterchangeSchemeName] [nvarchar] (50) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL,
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_InterchangeScheme_AuditLog] ON [dbo].[tbl_InterchangeScheme]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_InterchangeScheme'
      ...
      INSERT INTO dbo.tbl_AuditLogSource(TableName,PrimaryKeyField,PrimaryKeyValue,UpdatedDate,SourceType,SourceId) SELECT @TableName,'Id',COALESCE(i.Id,d.Id),@UpdatedDate,5,COALESCE(i.Id,d.Id) FROM inserted i FULL OUTER JOIN deleted d ON i.id=d.id
GO
ALTER TABLE [dbo].[tbl_InterchangeScheme] ADD CONSTRAINT [pk_InterchangeScheme] PRIMARY KEY NONCLUSTERED  ([Id]) ON [PRIMARY]
GO
