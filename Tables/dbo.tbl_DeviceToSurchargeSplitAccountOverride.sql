CREATE TABLE [dbo].[tbl_DeviceToSurchargeSplitAccountOverride]
(
...
[SplitType] [bigint] NOT NULL,
[SplitData] [bigint] NOT NULL,
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_DeviceToSurchargeSplitAccountOverride_AuditLog] ON [dbo].[tbl_DeviceToSurchargeSplitAccountOverride]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_DeviceToSurchargeSplitAccountOverride'
...
GO
ALTER TABLE [dbo].[tbl_DeviceToSurchargeSplitAccountOverride] ADD CONSTRAINT [pk_DeviceToSurchargeSplitAccountOverride] PRIMARY KEY CLUSTERED  ([Id]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_DeviceToSurchargeSplitAccountOverride_DeviceId_EndDate_StartDate_OverrideType_OverrideData] ON [dbo].[tbl_DeviceToSurchargeSplitAccountOverride] ([DeviceId], [EndDate], [StartDate], [OverrideType], [OverrideData]) ON [PRIMARY]
GO
