CREATE TABLE [dbo].[tbl_upm_ObjectToRole]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_upm_ObjectToRole_AuditLog] ON [dbo].[tbl_upm_ObjectToRole]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_upm_ObjectToRole'
...
GO
ALTER TABLE [dbo].[tbl_upm_ObjectToRole] ADD CONSTRAINT [pk_ObjectToRole] PRIMARY KEY CLUSTERED  ([id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_ObjectToRole_ObjectId] ON [dbo].[tbl_upm_ObjectToRole] ([ObjectId]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_ObjectToRole_RoleId] ON [dbo].[tbl_upm_ObjectToRole] ([RoleId]) ON [PRIMARY]
GO
