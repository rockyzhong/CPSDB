CREATE TABLE [dbo].[tbl_upm_ObjectToUser]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_upm_ObjectToUser_AuditLog] ON [dbo].[tbl_upm_ObjectToUser]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_upm_ObjectToUser'
...
GO
ALTER TABLE [dbo].[tbl_upm_ObjectToUser] ADD CONSTRAINT [pk_ObjectToUser] PRIMARY KEY CLUSTERED  ([Id]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_ObjectToUser_ObjectId] ON [dbo].[tbl_upm_ObjectToUser] ([ObjectId]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_ObjectToUser_UserId] ON [dbo].[tbl_upm_ObjectToUser] ([UserId]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
