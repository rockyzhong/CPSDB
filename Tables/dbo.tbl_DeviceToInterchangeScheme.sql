CREATE TABLE [dbo].[tbl_DeviceToInterchangeScheme]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_DeviceToInterchangeScheme_AuditLog] ON [dbo].[tbl_DeviceToInterchangeScheme]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_DeviceToInterchangeScheme'
...
GO
ALTER TABLE [dbo].[tbl_DeviceToInterchangeScheme] ADD CONSTRAINT [pk_DeviceToInterchangeScheme] PRIMARY KEY NONCLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [idx_DeviceToInterchangeScheme_DeviceId_InterchangeSchemeId] ON [dbo].[tbl_DeviceToInterchangeScheme] ([DeviceId], [InterchangeSchemeId]) ON [PRIMARY]
GO
