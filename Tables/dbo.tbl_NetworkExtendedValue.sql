CREATE TABLE [dbo].[tbl_NetworkExtendedValue]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_NetworkExtendedValue_AuditLog] ON [dbo].[tbl_NetworkExtendedValue]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_NetworkExtendedValue'
...
GO
ALTER TABLE [dbo].[tbl_NetworkExtendedValue] ADD CONSTRAINT [pk_NetworkExtendedValue] PRIMARY KEY NONCLUSTERED  ([Id]) ON [PRIMARY]
GO
