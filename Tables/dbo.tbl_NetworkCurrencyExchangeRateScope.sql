CREATE TABLE [dbo].[tbl_NetworkCurrencyExchangeRateScope]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_NetworkCurrencyExchangeRateScope_AuditLog] ON [dbo].[tbl_NetworkCurrencyExchangeRateScope]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_NetworkCurrencyExchangeRateScope'
...
GO
ALTER TABLE [dbo].[tbl_NetworkCurrencyExchangeRateScope] ADD CONSTRAINT [pk_NetworkCurrencyExchangeRateScope] PRIMARY KEY CLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_NetworkCurrencyExchangeRateScope_FromCurrency] ON [dbo].[tbl_NetworkCurrencyExchangeRateScope] ([FromCurrency]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_NetworkCurrencyExchangeRateScope_NetworkId_FromCurrency_ToCurrency] ON [dbo].[tbl_NetworkCurrencyExchangeRateScope] ([NetworkId], [FromCurrency], [ToCurrency]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_NetworkCurrencyExchangeRateScope_ToCurrency] ON [dbo].[tbl_NetworkCurrencyExchangeRateScope] ([ToCurrency]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_NetworkCurrencyExchangeRateScope_UpdatedUserId] ON [dbo].[tbl_NetworkCurrencyExchangeRateScope] ([UpdatedUserId]) ON [PRIMARY]
GO
