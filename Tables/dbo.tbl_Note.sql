...
[NoteText] [varchar] (511) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_Note_AuditLog] ON [dbo].[tbl_Note]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_Note'
...
GO
ALTER TABLE [dbo].[tbl_Note] ADD CONSTRAINT [pk_Note] PRIMARY KEY NONCLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_Note_SourceType_SourceId] ON [dbo].[tbl_Note] ([SourceType], [SourceId]) ON [PRIMARY]
GO
