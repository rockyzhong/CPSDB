CREATE TABLE [dbo].[tbl_DeviceToInterchangeSplitAccount]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_DeviceToInterchangeSplitAccount_AuditLog] ON [dbo].[tbl_DeviceToInterchangeSplitAccount]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_DeviceToInterchangeSplitAccount'
...
GO
ALTER TABLE [dbo].[tbl_DeviceToInterchangeSplitAccount] ADD CONSTRAINT [pk_DeviceToInterchangeSplitAccount] PRIMARY KEY NONCLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_DeviceToInterchangeSplitAccount_DeviceId_EndDate_StartDate_Recipient] ON [dbo].[tbl_DeviceToInterchangeSplitAccount] ([DeviceId], [EndDate], [StartDate], [Recipient]) ON [PRIMARY]
GO
