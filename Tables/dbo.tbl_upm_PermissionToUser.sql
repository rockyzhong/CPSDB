CREATE TABLE [dbo].[tbl_upm_PermissionToUser]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_upm_PermissionToUser_AuditLog] ON [dbo].[tbl_upm_PermissionToUser]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_upm_PermissionToUser'
...
GO
ALTER TABLE [dbo].[tbl_upm_PermissionToUser] ADD CONSTRAINT [pk_PermissionToUser] PRIMARY KEY CLUSTERED  ([Id]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_PermissionToUser_PermissionId] ON [dbo].[tbl_upm_PermissionToUser] ([PermissionId]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_PermissionToUser_UserId] ON [dbo].[tbl_upm_PermissionToUser] ([UserId]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
