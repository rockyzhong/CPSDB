CREATE TABLE [dbo].[tbl_DeviceFee]
(
...
[FeeFixed] [bigint] NOT NULL,
[FeePercentage] [bigint] NOT NULL,
[FeeAddedPercentage] [bigint] NOT NULL,
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_DeviceFee_AuditLog] ON [dbo].[tbl_DeviceFee]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_DeviceFee'
...
GO
ALTER TABLE [dbo].[tbl_DeviceFee] ADD CONSTRAINT [pk_DeviceFee] PRIMARY KEY CLUSTERED  ([Id]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_DeviceFee_DeviceId_StartDate_EndDate_AmountFrom_AmountTo] ON [dbo].[tbl_DeviceFee] ([DeviceId], [StartDate], [EndDate], [AmountFrom], [AmountTo]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_DeviceFee_IsoId_StartDate_EndDate_AmountFrom_AmountTo] ON [dbo].[tbl_DeviceFee] ([IsoId], [StartDate], [EndDate], [AmountFrom], [AmountTo]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
