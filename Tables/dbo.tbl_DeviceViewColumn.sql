CREATE TABLE [dbo].[tbl_DeviceViewColumn]
(
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_DeviceViewColumn_AuditLog] ON [dbo].[tbl_DeviceViewColumn]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_DeviceViewColumn'
...
GO
ALTER TABLE [dbo].[tbl_DeviceViewColumn] ADD CONSTRAINT [pk_DeviceViewColumn] PRIMARY KEY NONCLUSTERED  ([Id]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [idx_DeviceViewColumn_ViewId_ColumnId] ON [dbo].[tbl_DeviceViewColumn] ([ViewId], [ColumnId]) ON [PRIMARY]
GO
