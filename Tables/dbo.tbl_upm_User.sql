...
[BADGENAME] [nvarchar] (30) COLLATE SQL_Latin1_General_CP1_CI_AS NULL
) ON [PRIMARY]
GO
...
SET ANSI_NULLS ON
GO
CREATE TRIGGER [dbo].[tbl_upm_User_AuditLog] ON [dbo].[tbl_upm_User]     WITH EXECUTE AS 'dbo' FOR INSERT, UPDATE, DELETE NOT FOR REPLICATION
    AS
      SET NOCOUNT ON
      DECLARE @TableName varchar(200),@UpdatedDate varchar(21),@Type varchar(1),@FieldName varchar(200),@SQL nvarchar(4000)
      SELECT @TableName = 'tbl_upm_User'
      ...
      INSERT INTO dbo.tbl_AuditLogSource(TableName,PrimaryKeyField,PrimaryKeyValue,UpdatedDate,SourceType,SourceId) SELECT @TableName,'Id',COALESCE(i.Id,d.Id),@UpdatedDate,2,COALESCE(i.Id,d.Id) FROM inserted i FULL OUTER JOIN deleted d ON i.id=d.id
GO
ALTER TABLE [dbo].[tbl_upm_User] ADD CONSTRAINT [pk_User] PRIMARY KEY CLUSTERED  ([Id]) WITH (FILLFACTOR=80, PAD_INDEX=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_User_AddressId] ON [dbo].[tbl_upm_User] ([AddressId]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_User_IsoId] ON [dbo].[tbl_upm_User] ([IsoId]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [idx_User_ParentId] ON [dbo].[tbl_upm_User] ([ParentId]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [idx_User_UserName] ON [dbo].[tbl_upm_User] ([UserName]) ON [PRIMARY]
GO
